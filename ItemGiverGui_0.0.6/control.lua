        ...
        for _,item in pairs(get_inventory(player)) do
          if string.match(item.name, text) then
            if item.has_flag("hidden") and show_hidden(player) then
              ...
              elseif (list == 2) and (proto[item.name].type == "ammo") then
                table.insert(matches, {name = item.name, amount = item.amount})
              elseif (list == 3) and (proto[item.name].type == "armor") then
                table.insert(matches, {name = item.name, amount = item.amount})
              elseif (list == 4) and (proto[item.name].type == "capsule") then
                table.insert(matches, {name = item.name, amount = item.amount})
              elseif (list == 5) and (proto[item.name].type == "gun") then
                table.insert(matches, {name = item.name, amount = item.amount})
              elseif (list == 6) and (proto[item.name].type == "module") then
                table.insert(matches, {name = item.name, amount = item.amount})
              elseif (list == 7) and ((proto[item.name].type == "mining-tool") or (proto[item.name].type == "repair-tool") or (proto[item.name].type == "selection-tool")) then
                table.insert(matches, {name = item.name, amount = item.amount})
              end
    ...
    for _,item in pairs(items) do
      if string.match(item, text) then
        if proto[item].has_flag("hidden") and show_hidden(player) then
          ...
          elseif (list == 2) and (proto[item].type == "ammo") then
            table.insert(matches, {name = item, amount = math.floor(tonumber(amnt))})
          elseif (list == 3) and (proto[item].type == "armor") then
            table.insert(matches, {name = item, amount = math.floor(tonumber(amnt))})
          elseif (list == 4) and (proto[item].type == "capsule") then
            table.insert(matches, {name = item, amount = math.floor(tonumber(amnt))})
          elseif (list == 5) and (proto[item].type == "gun") then
            table.insert(matches, {name = item, amount = math.floor(tonumber(amnt))})
          elseif (list == 6) and (proto[item].type == "module") then
            table.insert(matches, {name = item, amount = math.floor(tonumber(amnt))})
          elseif (list == 7) and ((proto[item].type == "mining-tool") or (proto[item].type == "repair-tool") or (proto[item].type == "selection-tool")) then
            table.insert(matches, {name = item, amount = math.floor(tonumber(amnt))})
          end

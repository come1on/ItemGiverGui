...
script.on_event(defines.events.on_gui_checked_state_changed, function(event)
  ...
  if name == "igg_inv" or name == "igg_filter" or name == "igg_sort" then
    igg.update_gui(player)
  end
end)
...
function filter(player)
  return player.gui.center.igg_gui.igg_cmd_frame.igg_line2.igg_filter.state
end

function sort(player)
  return player.gui.center.igg_gui.igg_cmd_frame.igg_line2.igg_sort.state
end
...
function igg.open_gui(player)
  ...
  flow2.add({type = "label", caption = "inventory Items"})
  flow2.add({type = "checkbox", name = "igg_filter", state = true})
  flow2.add({type = "label", caption = "Filter Items"})
  flow2.add({type = "checkbox", name = "igg_sort", state = false})
  flow2.add({type = "label", caption = "Sort Items"})
...
function igg.update_gui(player, text, amnt)
  try_init()
  if not text then text = player.gui.center.igg_gui.igg_cmd_frame.igg_line1.igg_cmd_item.text end
  if not amnt then amnt = player.gui.center.igg_gui.igg_cmd_frame.igg_line1.igg_cmd_amount.text end
  if not (text == "") then
